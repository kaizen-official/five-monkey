            ...
            <div className="relative w-full flex overflow-hidden">
                ...
                >
                    {infiniteCards.map((card, index) => (
                        <div
                            key={`${card.id}-${index}`}
                            className={`w-72 h-96 md:w-80 md:h-[28rem] ${card.bgImage} bg-cover bg-center bg-no-repeat rounded-3xl shadow-2xl shrink-0 grayscale hover:grayscale-0 transition-all duration-500 hover:scale-105 border-4 border-[#FFF2E6]/10`}
                        >
                        </div>
                    ))}
                </motion.div>
