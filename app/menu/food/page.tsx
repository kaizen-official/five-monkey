    ...
    return (
        <div className="min-h-screen bg-[#FFF2E6] overflow-x-hidden pt-24 md:pt-32 pb-20 relative">
            <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-0">
                <Image src="/svg/Leaf.svg" alt="leaf" width={400} height={400} className="absolute top-10 right-0 opacity-5 rotate-12" />
            ...
            <div className="max-w-3xl mx-auto px-6 relative z-10">
                ...
                >
                    <h1 className="text-5xl md:text-6xl fontchange text-[#7E4300] mb-4">Dining</h1>
                    <div className="flex items-center justify-center gap-4">
                        <div className="h-0.5 w-12 bg-[#7E4300]/20"></div>
                        <span className="text-[#7E4300]/60 text-sm italic">Hearty. Flavorful. Authentic.</span>
                        <div className="h-0.5 w-12 bg-[#7E4300]/20"></div>
                    </div>
                </motion.div>
                ...
                <div className="space-y-16">
                    ...
                    {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                    {Object.entries(foodCategories).map(([key, category]: [string, any], index) => {
                        if (key === 'specialties') return null; // Skip specialties as it's handled above
...
const MenuSection = ({ title, items, delay }: { title: string, items: MenuItem[], delay: number }) => (
    ...
    >
        <h2 className="text-2xl md:text-3xl fontchange text-[#7E4300] mb-8 text-center sticky top-24 bg-[#FFF2E6]/95 backdrop-blur py-2 z-20">{title}</h2>
